...
if (!$image)
{
	echo json_encode([
		'success' => false,
		'message' => 'Imposible de trouver l\'image!'
	]);
	exit;
}
...
if (file_exists($imagePath))//Verification est ce que fichier existe
{
	if (!unlink($imagePath))//SI oui avec unlick on supprime le fichier
	{
		echo json_encode([
			'success' => false,
			'message' => 'Imposible de supprimer le fichier!'
		]);
		exit;
	}
}
