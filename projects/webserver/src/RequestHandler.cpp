...
HttpResponse RequestHandler::handleRequest(const HttpRequest &request, int serverPort, const std::string &serverIp, const std::string &serverHost)
{
    ...
    try
    {
        ...
        if (!location_config->redir.empty())
        {
            ...
            {
                HttpResponse res;
                if (request.getUri() == "/session")
                    res = _handleSessionExample(request);
                else if (request.getUri() == "/cookie")
                    res = _handleCookieExample(request);
                ...
                if (url[0] == '/')
                {
                    // Локальный редирект на свой же хост:порт
                    std::string hostPort;
                    std::map<std::string, std::string>::const_iterator hit = request.getHeaders().find("host");
                    if (hit != request.getHeaders().end() && !hit->second.empty())
                        hostPort = hit->second;
                    else if (!serverHost.empty())
                        hostPort = serverHost;
                    else
                        hostPort = serverIp;

                    bool hostHasPort = (hostPort.find(':') != std::string::npos);
                    std::string authority = hostPort;
                    if (!hostHasPort && serverPort != 80)
                    {
                        std::stringstream hs;
                        hs << hostPort << ":" << serverPort;
                        authority = hs.str();
                    }

                    std::stringstream ss;
                    ss << "http://" << authority << url;
                    res.addHeader("Location", ss.str());
                    return res;
                }
            }
...
{
    if (request.getUri() == "/api/session")
        return _handleSessionExample(request);
    if (request.getUri() == "/api/cookie")
        return _handleCookieExample(request);
...
HttpResponse RequestHandler::_handleSessionExample(const HttpRequest &request)
{
    HttpResponse response;
    ...
    if (it == cookies.end() || !SessionManager::hasSession(it->second))
    {
        std::string id = SessionManager::createSession();
        response.addCookie("SID", id, "Path=/");
        response.setBody("New session started. Visit count: 1");
    }
    else
    {
        size_t count = SessionManager::incrementSession(it->second);
        std::stringstream ss;
        ss << "Session id: " << it->second << "\nVisit count: " << count;
        response.setBody(ss.str());
    }
    return response;
}

HttpResponse RequestHandler::_handleCookieExample(const HttpRequest &request)
{
    HttpResponse response;
    response.addHeader("Content-Type", "text/plain");
    ...
    if (cookies.empty())
    {
        response.addCookie("flavor", "choco", "Path=/");
        response.setBody("No cookies found. Setting flavor=choco");
    }
    else
    {
        std::stringstream ss;
                                                  ...
                                                  const LocationStruct *location)
{
    ...
    if (!pagePath.empty())
    {
        ...
        std::string root = (location && !location->root.empty())
                               ? location->root
                               : server->rootDef;
        if (location->prefix != "/")//Здесь обрезаем префикс location , что бы получить доступ к папке с ошибками от корневого каталога.Не образаем если находимся изначально в корневом
...
{
    // 1) Сначала пробуем ТOЧНОЕ совпадение пути с префиксом
    ...
    // 2) Затем – самый длинный префикс, но только по "границе каталога":
    //    либо prefix == "/", либо prefix оканчивается на '/',
    //    либо следующая после префикса позиция в uri — это '/' (а не '.').
    ...
    for (size_t i = 0; i < server.location.size(); ++i)
    {
        ...
        if (uri.rfind(prefix, 0) == 0)
        {
            ...
            if (prefix == "/")
                boundary_ok = true;
            else if (!prefix.empty() && prefix[prefix.length() - 1] == '/')
                boundary_ok = true;
            else if (uri.length() == prefix.length())
                boundary_ok = true;
            else if (uri.length() > prefix.length() && uri[prefix.length()] == '/')
                boundary_ok = true;

            if (boundary_ok && prefix.length() > max_len)
            {
