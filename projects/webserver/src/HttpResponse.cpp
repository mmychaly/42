...
HttpResponse &HttpResponse::operator=(const HttpResponse &src)
{
    if (this != &src)
    {
        ...
        _cookies = src._cookies; // ВАЖНО: копируем Set-Cookie
        _body = src._body;
    }
    return *this;
}
...
void HttpResponse::addCookie(const std::string &name, const std::string &value, const std::string &attrs)
{
...
std::string HttpResponse::buildResponse() const
{
    ...
    {
        response_ss << "Set-Cookie: " << _cookies[i] << "\r\n";
    }
