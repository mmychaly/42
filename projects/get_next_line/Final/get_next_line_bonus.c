...
/*   Updated: 2024/09/25 18:24:05 by mmychaly         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (buffer == NULL)
		return (free_error(&line_buffer[fd]));
	buffer[0] = '\0';
	line_buffer[fd] = ft_read_fd(fd, line_buffer[fd], buffer);
	if (line_buffer[fd] == NULL)
		return (NULL);
	line_printable = ft_separator_line(line_buffer[fd]);
	if (!line_printable)
		return (free_error(&line_buffer[fd]));
	line_buffer[fd] = ft_next_string(line_buffer[fd]);
	return (line_printable);
}
