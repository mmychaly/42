...
int count_neightboards(t_game* game, int i, int i_2)
{
	int count = 0;
...
int play(t_game* game)
{
	...
	for (int i = 0; i < game->h; i++)
	{
		for (int i_2 = 0; i_2 < game->w; i_2++)
		{
			int neightboards = count_neightboards(game, i, i_2);
			if (game->board[i][i_2] == game->alive)
			{
				if (neightboards == 2 || neightboards == 3)
...
void fill_board(t_game* game)
{
	...
	while (read(STDIN_FILENO, &buffer, 1) == 1)
	{
		...
		if (game->draw && flag == 0)
		{
			if (game->y >= 0 && game->y < game->h && game->x >= 0 && game->x < game->w)
